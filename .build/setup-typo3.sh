...
export DBNAME=nginx_cache_test_db
...
then
	.build/vendor/bin/typo3cms install:setup --non-interactive --database-user-name="root" $PWARG --database-host-name="localhost" --database-port="3306" --database-name="$DBNAME" --admin-user-name="admin" --admin-password="password" --site-name="Test Install" --site-setup-type="createsite"
else
